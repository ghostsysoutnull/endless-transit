...
2. Read the verb. *Why / tell me / explain* → answer. *Your take* → a short take with a marked pick on what is still
   open, starting at the concept, not the code. *Expand X* → read the code, bring options, each with its change, cost
   and edge. *Execute / do it / commit / push* → exactly that scope, then report.
...
6. One question per message, lettered options, the pick first as `(A) ★ …` — never as prose ending "want me to…?".
   A confirm option names its scope: the files, the kind of edit, and what is not touched. Design options are
   ambitious. A process option names its cost, what it teaches that isn't already known, and the failure it prevents.
   A pick is built, not re-asked.
