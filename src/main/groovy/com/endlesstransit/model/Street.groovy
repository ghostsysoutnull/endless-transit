...
class Street extends Container {
    ...
    List<String> getExtraContent(Player player, int width) {
        ...
        lines << fmt.dim("Buildings on this street:")
        lines << fmt.dim("-" * width)
        ...
        for (int i = 0; i < bldgs.size(); i += 2) {
            ...
            if (bL.isLandmark) nameL = fmt.colorize(fmt.bold(nameL), "CYAN")
            String visL = bL.isVisited() ? fmt.colorize(" [V]", "GREEN") : ""
            String labelL = "${numL}. ${nameL}${visL}"
            String leftPart = fmt.padRight(labelL, colWidth)

            String labelR = ""
            if (bR != null) {
                String bNameR = bR.getName()
                String nameR = bNameR.length() > 25 ? bNameR.substring(0, 22) + "..." : bNameR
                if (bR.isLandmark) nameR = fmt.colorize(fmt.bold(nameR), "CYAN")
                String visR = bR.isVisited() ? fmt.colorize(" [V]", "GREEN") : ""
                labelR = "${numR}. ${nameR}${visR}"
            }

            lines << (leftPart + " | " + fmt.padRight(labelR, colWidth))
        }
        lines << fmt.dim("-" * width)
        return lines
    }
    ...
    String getDescription() {
        ...
        String vInfo = v != null ? " | ${fmt.dim("[TECH_ERA:")} ${fmt.colorize(v.timeline.toUpperCase(), "YELLOW")}${fmt.dim("]")} ${fmt.dim("[RESONANCE:")} ${fmt.colorize(v.primaryCulture.toUpperCase(), v.atmosphericColor)}${fmt.dim("]")}" : ""
