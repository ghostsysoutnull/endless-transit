...
class HeadlessSimulationTest {
    static void main(String[] args) {
        ...
        ScreenBuffer finalScreen = HeadlessRunner.run(seed, script)
        ...
        try {
            VisualAssertionEngine.expect(finalScreen).with {
                isBoxedCorrectly()
                hasLayout("Corridor")
                contains("LATTICE_SYNC: [NOMINAL]")
            }
            Terminal.println Terminal.colorize("SUCCESS: Visual integrity verified via DSL.", Terminal.GREEN)
