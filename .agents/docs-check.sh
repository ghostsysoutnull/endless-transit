...
# Usage: .agents/docs-check.sh [--agent]      (normally via ./vinc.sh --docs [--agent])
...
if [ -n "$VINC_DISCOVERED" ]; then
    ...
    ACTUAL=$(./vinc.sh --test --agent 2>/dev/null | sed -n 's/.*STATUS=PASS DISCOVERED=\([0-9]*\).*/\1/p')
fi
...
# --- D3: blueprint stamps --------------------------------------------------------------------------------------------
D3=ok; BP_COUNT=0
for BP in "$ROOT"/docs/blueprints/logic/classes/*/*.md; do
    [ -f "$BP" ] || continue
    BP_COUNT=$((BP_COUNT + 1))
    PKG=$(basename "$(dirname "$BP")"); CLS=$(basename "$BP" .md)
    SRC="$ROOT/src/main/groovy/com/endlesstransit/$PKG/$CLS.groovy"
