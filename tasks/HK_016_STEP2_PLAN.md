...
| c4 | **2d** timeline drift | `VibeCapsule`, `PlanetFactory`, `ApartmentFactory`, `CityFactory` (4); `RegionalDivergenceTest` +1 assertion; `:155` re-pin if the 0x1234 apartment rolls secondary; pin `secondaryTimeline_drawnAndPicked` | goldens 16/17/30 (`TEMPORAL_MARKER`, lighting line, objects) only if the golden apartment rolls secondary; simulated first |
