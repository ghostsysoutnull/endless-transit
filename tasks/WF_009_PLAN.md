...
**Status:** PLAN, amended once before the grill. **User decisions (2026-09-20):** options 1 + 2 (plan declares shape, grill interrogates it, one lint rule); option 3 (post-implementation review pass) not taken. **D2 (user question "should the law not be more generic?"):** yes — the law states the *principles* (eight, one line each, pointer to the wave that taught it), the grill asks only the questions that have a verifiable form; incidents are where principles were noticed, not the law itself.
