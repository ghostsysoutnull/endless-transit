...
- `run.sh:118` (`--test`) runs `src/test/groovy/com/endlesstransit/AllTests.groovy`, which does not exist (the runner is `TestRunner.groovy`, via `vinc.sh`); `run.sh --help` advertises `--headless`, which no code reads. Two documents still send the reader to the dead command: `tasks/skills/skill-chronicle/SKILL.md:28` (`./run.sh --test --seed-scan`) and `tasks/backlog/AI_STRATEGY_IMPLEMENTATION_PLAN.md:24` — fix them with the script.
