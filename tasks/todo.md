...
- [ ] **ACTIVE: HK-016 step 1** — fill the silent resource files (lighting ×4, structures ×3, walls ×5, lexicons ×4), index-loaded lexicon, `[THEME_WARN]` on any fallback, glitch key fix, `ThemeResourceCoverageTest`, docs. Plan: `tasks/HK_016_STEP1_PLAN.md` (grilled, simulated). Branch `content/hk-016-step1`.
- [ ] After step 1: **HK-016 step 2** (structural: shuffled deck, furniture ≠ objects, timeline drift, phrasings, category names, description variants), then **step 3** (grow lists). Other open items: **HK-015** (player-facing bugs; items 1–2 are gameplay changes), O1 (HeadlessRunner DSL), HK-013 (nine long methods)
