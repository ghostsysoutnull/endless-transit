...
*   **Dropping into a cell:** From a room's `[t]` menu, `d[ID]` releases a fragment onto the floor of that cell. The cell remembers it across synchronizations — but it remembers only the **name**. Re-captured, the fragment resonates at the frequency its name spells in that cell. A relic taken back from the same cell returns unchanged. A Hybrid returns without its sum, a Hidden Frequency without its millions, and a **Keystone** returns as a common fragment that opens nothing. Leave only relics on the floor.
...
## [THE "HYBRID" OVERFLOW]
A hybrid takes the **first word** of each parent, joined with a dash: "Rusted Chain" and "Paper Lantern" become **Rusted-Paper Hybrid**. Recursive merging keeps only the first word again, so long chains converge on the same few names. The buffer badges each hybrid with its own lineage: `NEW_SYNTHESIS` for a first merge, `SYNTHESIS_x<n>` once it carries the merges of its parents. The badges last one visit: leaving the buffer screen clears them all. Nothing shatters.
