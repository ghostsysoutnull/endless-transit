...
- **Atmosphere structures index uses mixed case (`Agricultural`, `Ceremonial`, `Military`, `Research`).** These match the filenames exactly, which have capitalised names. If a new structure file is added with lowercase, the index pattern will be inconsistent. Not urgent, but worth normalising if Phase 9 (ProceduralFactory Split) adds new structure categories.
