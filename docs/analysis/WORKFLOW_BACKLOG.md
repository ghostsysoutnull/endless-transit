...
New lessons are rule + pointer (CODEX, Self-Improvement Loop); the older bullets in `tasks/lessons/*.md` are paragraphs loaded every session. **Decide:** rewrite them as rule + pointer. Needs bullet-by-bullet user review, because a shortened lesson can lose the rule. User call, deferred. **2026-09-25: CLOSED** — widened to every always-loaded rule and done: `tasks/completed/RULES_DIET_PLAN.md` (always-loaded words ≈ 6,500 → ≈ 2,900; the user rules in one block at the top of `CLAUDE.md`).
