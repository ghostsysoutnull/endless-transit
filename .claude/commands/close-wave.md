...
| :--- | :--- | :--- | :--- |
| 1 | **Preconditions** | Code merged to `master` (the chronicle id is that merge's hash). A wave with **no `src/` change** may close on its own branch instead — the chronicle id is its last work commit and one merge carries both. `./vinc.sh --test --agent`, `./vinc.sh --lint --agent`, and `./vinc.sh --scan` if `model`/`procgen` changed. `git status --short` clean (the player's untracked save backups excepted). | the three output lines |
