...
assumes a document is stale and looks for which one. Two halves: `./vinc.sh --docs` owns the facts a machine
...
| **Light** | nothing under `src/` | 1, 2, 8, 9, 10 |
| **Full** | anything under `src/`, a moved golden, a changed `config/lint/baseline.xml` | all ten |
...
| law or tooling changed (`.claude/`, any `CLAUDE.md`, `vinc.sh`, `.agents/`, `config/lint/`) | 5 Chronicle |
...
| 6 | **Retro** | `docs/retro/RETRO_<WAVE>.md` — what went well / wrong / concerns, not a second changelog. Friction → `WORKFLOW_BACKLOG.md`; a `High` item blocks the next phase. | file, items logged |
...
| 10 | **Gate** | `./vinc.sh --docs --agent` → `DOCS=PASS`. A FAIL names the file and the stale value. Never edit a stamp, a count or a word total to match without doing the row it belongs to. | the output line |
...
- One branch (`docs/close-<wave>`, or the wave's own branch when it had no `src/` change). Scripted edits and
